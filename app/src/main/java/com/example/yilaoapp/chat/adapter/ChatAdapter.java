package com.example.yilaoapp.chat.adapter;
...
import com.example.yilaoapp.chat.bean.MsgBody;
...
import com.example.yilaoapp.chat.util.GlideUtils;
...
public class ChatAdapter extends BaseQuickAdapter<Message, BaseViewHolder> {
    ...
    private void setContent(BaseViewHolder helper, Message item) {
        if (item.getMsgType().equals(MsgType.TEXT)) {
            GlideUtils.loadChatImage(mContext, item.getUuid(), (ImageView) helper.getView(R.id.chat_item_header));
